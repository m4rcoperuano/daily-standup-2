...
export function useApi() {
  ...
  const teamSettings = {
    integrations: {
      save: async ( clockwork_api_key: string ): Promise<CustomResponse> => {
        return await callApi( 'post', route( 'clockwork.settings-update' ), { clockwork_api_key } );
      },
      get: async (): Promise<CustomResponse> => {
        return await callApi( 'get', route( 'clockwork.settings' ) );
      },
    },
  };
