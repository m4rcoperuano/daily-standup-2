...
const save = async () => {
    const response = await axios.post(route('stand-up-entries.store'), form.value);
    emits('saved');
};
    ...
    <div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-teal-100 dark:bg-teal-950 px-4 py-2 sm:rounded-lg">
                ...
                <textarea v-model="form.in_progress" placeholder="- Completed task A" rows="5"></textarea>
            </div>
            <div class="bg-blue-100 dark:bg-blue-950 px-4 py-2 sm:rounded-lg">
                <div class="mb-2">
                    Priorities
                </div>
                <textarea v-model="form.priorities" placeholder="- Will complete task B" rows="5"></textarea>
            </div>
            <div class="bg-red-100 dark:bg-red-950 px-4 py-2 sm:rounded-lg">
                <div class="mb-2">
                    Blockers
                </div>
                <textarea v-model="form.blockers" placeholder="- Waiting on task C" rows="5"></textarea>
            </div>
        </div>
